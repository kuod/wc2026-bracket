...
- **Late-pick guard (currently off).** The scorer can dock picks made suspiciously late — i.e. submitted more than 24 hours after a result was already knowable — replacing their credit with a "crowd-share" value (how much of the pool also picked that winner) instead of full points, so hindsight upset-calling doesn't pay off. This is implemented but disabled (`CHEAT_GUARD_ENABLED = false` in `score-app.js`) for this friendly pool; flip it on if a score is ever contested.
